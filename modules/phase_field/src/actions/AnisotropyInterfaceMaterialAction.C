...
AnisotropyInterfaceMaterialAction::validParams()
{
    ...
    params.addClassDescription("Set up materials for the InterfaceOrientationMultiphaseMaterial "
    ...
    params.addParam<MaterialPropertyName>("kappa_name", "kappa", "speficies the base name for the gradient energy coefficient");
    params.addParam<MaterialPropertyName>("dkappadgrad_eta_name", "dkappadgrad", 
    "specifies the base name for the first derivative of gradient energy coefficient with respect to grad etas");
    params.addParam<MaterialPropertyName>("d2kappadgrad_eta_name", "d2kappadgrad_",
    "specifies the base name for the second derivative of gradient energy coefficient with respect to grad etas");
    ...
    params.addParam<std::vector<Real>>("reference_angle", "Reference angle for defining anisotropy in degrees with respect to the liquid");
    params.addParam<Real>("kappa_bar", 0.1125, "Average value of the interface parameter kappa");
    params.addParam<std::vector<std::string>>("output_kappa_name",
    "Name of material property for the combined kappa and kappa gradients");
    params.addParam<VariableName>("reference_phase", "the phase that will be the reference for the angle");
    return params;
}

AnisotropyInterfaceMaterialAction::AnisotropyInterfaceMaterialAction(const InputParameters & parameters)
 : Action(parameters)
{
}
...
AnisotropyInterfaceMaterialAction::act()
{
    ...
    const auto reference_phase = getParam<VariableName>("reference_phase");

    // Size definitions for etas
    unsigned int n_etas = etas.size();
    unsigned int n_angles = reference_angle.size();
    ...
    for (unsigned int i = 0; i < n_etas; ++i)
    {
        ...
        for (unsigned int j = 0; j < n_etas; ++j)
        {
            if (i != j)
            {
                ...
                params.set<MaterialPropertyName>("kappa_name") = kappa_name+'_'+etaa_name+'_'+etab_name;
                params.set<MaterialPropertyName>("dkappadgrad_etaa_name") = dkappadgrad_eta_name+'_'+etaa_name+'_'+etab_name;
                params.set<MaterialPropertyName>("d2kappadgrad_etaa_name") = d2kappadgrad_eta_name+'_'+etaa_name+'_'+etab_name;
    ...
    InputParameters params = _factory.getValidParams("GrandPotentialAnisoInterface");
    params.set<std::vector<std::string>>("output_properties") = output_kappa_name;
    params.set<std::vector<VariableName>>("etas") = etas;
    _problem -> addMaterial("GrandPotentialAnisoInterface", "GP_aniso_interface", params);
}